...
There are two functions in this file that will provide the value for any
position in the fibinacci sequence. One is strictly recursive while the other
is recursive but takes advantage of memoization techniques to speed things up
dramatically.
...
What you will find out is that with smaller numbers in the sequence recursion
on its own is much faster than using memoization mostly because of the overhead
...
def fib_recursive(n):
    ...
    return(int(fib_recursive(n - 1)) +
           int(fib_recursive(n - 2)))
...
def fib(number, memo={}):
    ...
    memo[number] = int(fib(number - 1, memo)) + int(fib(number - 2, memo))
